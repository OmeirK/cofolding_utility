...
from rdkit.Chem import rdFMCS, AllChem, ChemicalFeatures
...
parser.add_argument('--frag_ensemble', '-fe', help='Path to a .mol file with the ensemble of all fragment hits. Must be in the same frame of reference as --ref_rec')
parser.add_argument('--fragment_sdf', '-fsdf', help='Path to an sdf with the subset of fragment structures to use for mcs_cov calculations. NOTE: all structures should be prealigned to --ref_rec.', default=None)
parser.add_argument('--fragment_list', '-fl', help='Instead of providing --fragment_sdf, provide a file with paths to fragalysis directories for fragment hits. Requires folder to be named {frag-id}/, and contain {frag-id}.pdb and a {frag-id}_ligand.sdf file (default = None)', default=None)
        ...
        for s in seeds:
            ...
            for i in range(1,6):
                ...
                if os.path.exists(model_path):
                    model_ligs = glob.glob(f'{cr}/{s}/{case_n}_{s}_sample_{i}_*lig.sdf')
                    of3_data[case_n][s][i] = {'cif': model_path, 'sdfs': model_ligs}
                    #print('\t', cr, s, i, os.path.exists(model_path))
                    
                    #for sdf in model_ligs:
                        #mol = Chem.MolFromMolFile(sdf)
                        #if mol is not None:
                        #    mol_smi = Chem.MolToSmiles(mol)
                        #    mol.SetProp('_Name', f'{case_n}-{s}-{i}')
                        #    mol.SetProp('smi', mol_smi)
                        #    mol.SetProp('path', sdf)

                        #    of3_data[case_n][s][i]['mols'].append(mol)
    ...
    for case in of3_struct_data:
        ...
        for seed in of3_struct_data[case]:
            ...
            if len(aligned_models) == 0:
                aligned_models, invalid_models = check_frag_alignment(of3_struct_data[case][seed], args.frag_ensemble, args.ref_rec, tmpdir=args.tmpdir)
